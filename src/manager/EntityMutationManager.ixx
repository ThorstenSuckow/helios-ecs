...
class EntityMutationManager {
    ...
    public:
        ...
        void add(std::vector<TCommandType>& incoming) {
            ...
            if (commands_.empty()) {
                ...
                commands_.insert(
                    commands_.end(), std::make_move_iterator(incoming.begin()), std::make_move_iterator(incoming.end())
                );
                incoming.clear();
            }
        }
    ...
    auto* executorFor() {
        ...
        if (!executor) {
            auto& created = internalExecutionManagerRegistry_.template add<InternalExecutionManager<TCommand>>();

            using Component_type = TCommand::Component_type;
            ...
            componentToInternalExecutorGroups_[cv].push_back(
                manager::types::ManagerTypeId::template id<InternalExecutionManager<TCommand>>()
            );
...
public:
    ...
    bool init(CommandHandlerRegistry& commandHandlerRegistry, EntityManager<THandle>& entityManager) noexcept {
        ...
        commandHandlerRegistry
            .registerHandlerForCommandGroup<command::types::CommandGroup<commands::RemoveComponentCommand, THandle>>(
                *this
            );
