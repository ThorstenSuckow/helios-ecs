...
public:
    ...
    template <typename THandle>
    explicit HandleSpanRef(const std::vector<THandle>& handles) : HandleSpanRef{std::span<const THandle>{handles}} {}

    template <typename THandle>
    explicit HandleSpanRef(std::span<const THandle> handles)
        : handleTypeId_(HandleTypeId::template id<THandle>()), data_(handles.data()), size_(handles.size()) {}
    ...
    std::span<const THandle> get() const {
        ...
        return {static_cast<const THandle*>(data_), size_};
    }
};
