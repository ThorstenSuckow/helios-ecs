 ...
 */
enum class SortCriteria { MaxEntityId, ComponentCount };
...
public:
     ...
     */
    explicit EntityManager(const size_t capacity = 0) : capacity_(capacity) {
        reserve(capacity);
    }
    ...
    [[nodiscard]] bool destroy(const HandleType handle) {
        ...
        for (const auto & component : components_) {
            ...
            component->remove(handle.entityId());
        }
    ...
    /**
     * @brief Resets the specified targetHandle's component set to the components of the entity represented by
     * sourceHandle.
     *
     ...
     */
    [[nodiscard]] bool
    resetTo(const HandleType targetHandle, const EntityManager& source, const HandleType sourceHandle) {
        ...
        forEachComponentTypeId(targetHandle, [&](const ComponentTypeIdType typeId) {
            ...
            called = true;
            allReset &=
                sparseSet(typeId)->resetTo(targetHandle.entityId(), *source.sparseSet(typeId), sourceHandle.entityId());
        });
    ...
    HandleType copyFrom(EntityManager& sourceEntityManager, const HandleType sourceHandle) {
        ...
        sourceEntityManager.forEachComponentTypeId(sourceHandle, [&](const ComponentTypeIdType typeId) {
            ...
            allCopied =
                allCopied && source->copyTo(sourceHandle.entityId(), *components_[idx], targetHandle.entityId());
            called = true;
        });
    ...
    void sort(std::vector<SparseSetBase*>& sparseSets, const SortCriteria sortCriteria) noexcept {
        ...
        case SortCriteria::MaxEntityId:
            std::sort(sparseSets.begin(), sparseSets.end(), [this](const auto a, const auto b) noexcept {
